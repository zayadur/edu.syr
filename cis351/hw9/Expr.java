...
public class Expr {
  ...
  /** Evaluates an expression tree.
    ...
    **/
    ...
    /* 
     * Could not figure out why x and y assume their values
     */

  public double eval(HashMap<Character,Double> env) {
    switch(tag) {
      case NUM: return num; // cannot do anything with num, return it
      case VAR: return name; // cannot do anything with var, return it
      default:
        switch(name) { // switch cases based on operand (name)
        case '+': return left.eval(env) + right.eval(env);
        case '-': return left.eval(env) - right.eval(env);
        case '*': return left.eval(env) * right.eval(env);
        case '/': return left.eval(env) / right.eval(env);
        default: return eval(env);
      }
    }
  }
  ...
  /** Converts the expression tree to the infix form of the expression.
    ...
    **/
  public String toInfix() { // form: num op var
    switch(tag) {
      case NUM: return " " + num + " ";
      case VAR: return " " + name + " ";
      default: return " ( " + left.toInfix() + name + right.toInfix() + " ) ";
    }
  }
  ...
  /** Converts the expression tree to a postfix form of the expression.
    ...
    **/
  public String toPostfix() { // form: num var op
    switch(tag) {
      case NUM: return " " + num + " ";
      case VAR: return " " + name + " ";
      default: return " " + left.toPostfix() + right.toPostfix() + name + " ";
    }
  }  
  ...
  public static Expr subst(Expr e1, char v, Expr e2) {
    switch(e1.tag) {
      case NUM: return e1; // if e1 is a number, e1 is returned
      case VAR: // if e1 is a variable, then: 
  ...
  public Expr simplify() {
    switch(tag) {
      case NUM: return new Expr(num);
      case VAR: return new Expr(name);
      default:
        // not working
        return new Expr(0.0);
    }
